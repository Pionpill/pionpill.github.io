...
<p class="tip">构建 workInProgress Fiber 树时会复用 current Fiber 树中已有的 Fiber 节点(通过 Diff)，所以占用的内存也不多。</p>
...
这两个方法都会开启 render 与 commit 流程，虽然同步和异步处理有些许不同，但整体上回调用以下几个核心方法:
- render 方法: 同步的 `renderRootSync` 与异步的 `renderRootConcurrent`
- commit 方法: 都会调 `commitRoot` 方法，异步逻辑的 `finishConcurrentRender` 方法里回调这个方法

### 同步构建过程

我们先简单看一下 `performSyncWorkOnRoot`:
...
看一下 `performConcurrentWorkOnRoot`:
